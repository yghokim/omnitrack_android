 ...
 */
class ChoiceInputView(context: Context, attrs: AttributeSet? = null) : AFieldInputView<IntArray>(R.layout.input_choice, context, attrs) {

    private val listView: RecyclerView = findViewById(R.id.ui_recyclerview)
    ...
    private inline fun modifyEntryList(modifyFunc: (UniqueStringEntryList) -> Boolean) {
        if (boundAttributeObjectId != null) {
            this.realmProvider.get().use { realm ->
                val attribute = realm.where(OTFieldDAO::class.java).equalTo(BackendDbManager.FIELD_OBJECT_ID, boundAttributeObjectId).findFirst()
                if (attribute != null) {
                    val helper = attribute.getHelper(context) as OTChoiceFieldHelper
                    val originalEntryList = helper.getChoiceEntries(attribute)
                    if (originalEntryList != null) {
                        if (modifyFunc(originalEntryList)) {
                            realm.executeTransaction {
                                helper.setPropertyValue(OTChoiceFieldHelper.PROPERTY_ENTRIES, originalEntryList, attribute, realm)
                            }
