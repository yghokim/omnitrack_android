...
class FieldDetailActivity : MultiButtonActionBarActivity(R.layout.activity_field_detail), View.OnClickListener {

    companion object {
        const val INTENT_EXTRA_SERIALIZED_ATTRIBUTE_DAO = "serializedAttributeDao"

        fun makeIntent(context: Context, dao: OTFieldDAO): Intent {
    ...
    private lateinit var viewModel: FieldDetailViewModel
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        currentSpinnerEntries = arrayListOf(
                FallbackSpinnerEntry(OTFieldDAO.DEFAULT_VALUE_POLICY_NULL, getString(R.string.msg_attribute_fallback_policy_null)),
                FallbackSpinnerEntry(OTFieldDAO.DEFAULT_VALUE_POLICY_FILL_WITH_LAST_ITEM, getString(R.string.msg_attribute_fallback_policy_last)),
                FallbackSpinnerEntry(OTFieldDAO.DEFAULT_VALUE_POLICY_FILL_WITH_PRESET, getString(R.string.msg_attribute_fallback_policy_preset))
        )

        viewModel = ViewModelProviders.of(this).get(FieldDetailViewModel::class.java)
        ...
        nameProperty.isEnabled = viewModel.fieldDAO?.isEditNameEnabled() == true
        requiredProperty.isEnabled = viewModel.fieldDAO?.isRequiredToggleEnabled() == true
        ui_connection_group.isEnabled = viewModel.fieldDAO?.isEditMeasureFactoryEnabled() == true
        ui_connection_group.alpha = if (viewModel.fieldDAO?.isEditMeasureFactoryEnabled() == true) InterfaceHelper.ALPHA_ORIGINAL else InterfaceHelper.ALPHA_INACTIVE

        ui_property_list.isEnabled = viewModel.fieldDAO?.isEditPropertyEnabled() == true
        ui_property_list.alpha = if (viewModel.fieldDAO?.isEditPropertyEnabled() == true) InterfaceHelper.ALPHA_ORIGINAL else InterfaceHelper.ALPHA_INACTIVE

        ui_fallback_policy_container.isEnabled = viewModel.fieldDAO?.isEditPropertyEnabled() == true
        ui_fallback_policy_container.alpha = if (viewModel.fieldDAO?.isEditPropertyEnabled() == true) InterfaceHelper.ALPHA_ORIGINAL else InterfaceHelper.ALPHA_INACTIVE
        ...
        creationSubscriptions.add(
                viewModel.typeObservable.subscribe { type ->
                    ...
                    try {
                        val attrHelper = fieldManager.get(type)
                        ...
                        for (entryWithIndex in fieldViewFactoryManager.get().get(type).makePropertyViews(this).withIndex()) {
                        ...
                        if (viewModel.isValid || viewModel.fieldHelper?.propertyKeys?.isEmpty() != false) {
                            //no property
                            ui_property_list.setBackgroundResource(R.drawable.bottom_separator_light)
                        } else if (viewModel.fieldHelper?.propertyKeys?.size == 1) {
                            //single property
    ...
    private fun refreshFallbackSpinner(dao: OTFieldDAO, attrHelper: OTFieldHelper) {
    ...
    private fun checkIsChangedAndIndicate() {
        columnNameView.showEditedOnTitle = columnNameView.value != field?.name
        if (field?.valueConnection != connectionView.connection) {
        ...
        for (entry in propertyViewList) {
            if (entry.second is APropertyView<*>) {
                entry.second.showEditedOnTitle = field?.getPropertyValue<Any>(entry.first) != entry.second.value!!
            }
        }
    }*/
/*
    private fun isChanged(): Boolean {
        ...
        if (columnNameView.value != field?.name) {
            return true
        }
        ...
        for (entry in propertyViewList) {
            if (entry.second is APropertyView<*>) {
                if (field?.getPropertyValue<Any>(entry.first) != entry.second.value!!) {
                    return true
                }
            }
        }
    ...
    private fun saveChanges(): Boolean {
        if (isAllSettingsValid()) {
            ...
            viewModel.fieldDAO?.let {
                if (dirtySignature != null) {
                    eventLogger.get().logAttributeChangeEvent(IEventLogger.SUB_EDIT, viewModel.fieldDAO?.localId
                            ?: "", viewModel.fieldDAO?.trackerId) { content ->
