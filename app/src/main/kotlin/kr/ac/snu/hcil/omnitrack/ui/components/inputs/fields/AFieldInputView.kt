 ...
 */
abstract class AFieldInputView<DataType>(layoutId: Int, context: Context, attrs: AttributeSet? = null) : AInputView<DataType?>(layoutId, context, attrs), IActivityLifeCycle, LifecycleObserver {
    ...
    companion object {
        ...
        fun makeInstance(type: Int, context: Context): AFieldInputView<out Any> {
            return when (type) {
                ...
                else -> throw IllegalArgumentException("field view data type $type is not supported yet.")
            }
        }
