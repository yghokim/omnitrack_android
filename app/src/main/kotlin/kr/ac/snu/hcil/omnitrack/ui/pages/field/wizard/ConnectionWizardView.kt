package kr.ac.snu.hcil.omnitrack.ui.pages.field.wizard
...
import kr.ac.snu.hcil.omnitrack.ui.pages.field.wizard.pages.SourceConfigurationPage
import kr.ac.snu.hcil.omnitrack.ui.pages.field.wizard.pages.SourceSelectionPage
import kr.ac.snu.hcil.omnitrack.ui.pages.field.wizard.pages.TimeQueryPage
...
class ConnectionWizardView : WizardView {
    ...
    fun init(field: OTFieldDAO) {
        init(field, OTConnection())
    }

    /**
     * used to modify existing connection
     */
    fun init(field: OTFieldDAO, connection: OTConnection) {
        setAdapter(Adapter(field, context))
    ...
    inner class Adapter(field: OTFieldDAO, context: Context) : AWizardViewPagerAdapter(context) {
        val pages = Array(3) {
            index ->
            when (index) {
                PAGE_INDEX_SOURCE_SELECTION -> SourceSelectionPage(this@ConnectionWizardView, field)
