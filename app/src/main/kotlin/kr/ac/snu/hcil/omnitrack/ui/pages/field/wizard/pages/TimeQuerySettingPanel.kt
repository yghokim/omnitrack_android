...
class TimeQuerySettingPanel : LinearLayout, IEventListener<Int> {
    ...
    private fun buildPivotEntries() {
        ...
        /*
        val siblingAttributes = this.field?.tracker?.fields
        if (siblingAttributes != null) {
            ...
            for (field in siblingAttributes) {
                if (field !== this.field) {
                    when (field.typeId) {
                        OTFieldManager.TYPE_TIME -> {
                            pivotEntryAdapter.add(
                                    AttributePivotEntry(String.format(fieldNameFormat, field.name), OTTimeRangeQuery.TYPE_PIVOT_TIMEPOINT, field)
                            )
                        }
                        OTFieldManager.TYPE_TIMESPAN -> {
                            pivotEntryAdapter.add(
                                    AttributePivotEntry(String.format(fieldNameFormat, field.name), OTTimeRangeQuery.TYPE_LINK_TIMESPAN, field)
                            )
                        }
                    }
                }
